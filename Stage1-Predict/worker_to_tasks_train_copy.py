...
from dataloader import load_graph_adj_mtx, load_task_graph_node_features
from model import *
...
def Tasktrain(args):
    ...
    # Build task graph (built from train_df)
    print('Loading task graph...')
    raw_A = load_graph_adj_mtx(args.task_data_adj_mtx)
    raw_X = load_task_graph_node_features(args.task_data_node_feats,
                                     args.feature1,
                                     args.feature2,
                                     args.feature3,
                                     args.feature4)
    ...
    X[:, 0] = raw_X[:, 0]
    # print(f'raw_X[:, 0]: {raw_X[:, 0].shape}', f'X.shape: {X.shape}')
    X[:, 1:num_cats + 1] = one_hot_rlt
    X[:, num_cats + 1:] = raw_X[:, 2:]
    print(X, X.shape)
    print(type(X))
    logging.info(f"After one hot encoding poi cat, X.shape: {raw_X.shape}")
    ...
    # POI id to index``
    nodes_df = pd.read_csv(args.task_data_node_feats)
    ...
    embed_fuse_model1 = TaskFuseEmbeddings(args.user_embed_dim, args.poi_embed_dim, args.cat_embed_dim, args.time_embed_dim)

    # %% Model6: Sequence model
    args.seq_input_embed = args.poi_embed_dim + args.user_embed_dim + args.cat_embed_dim + args.time_embed_dim
    ...
    def input_traj_to_embeddings(sample, poi_embeddings):
        ...
        for idx in range(len(input_seq)):
            ...
            fused_embedding1 = embed_fuse_model1(user_embedding, poi_embedding, cat_embedding, time_embedding)
    ...
    for epoch in range(args.epochs):
        ...
        for b_idx, batch in enumerate(train_loader):
            ...
            y_pred_poi, y_pred_cat, y_pred_time = seq_model(x, src_mask)
            ...
            # Final loss
            loss = loss_poi + loss_cat + loss_time * args.time_loss_weight
            optimizer.zero_grad()
            ...
            if (b_idx % (args.batch)) == 0:
                ...
                logging.info(f'label_seq_cat:{[poi_idx2cat_idx_dict[each[0]] for each in batch[sample_idx][2]]}')
                logging.info(f'pred_seq_cat:{list(np.argmax(batch_pred_cats, axis=2)[sample_idx][:batch_seq_lens[sample_idx]])}')
                logging.info(f'label_seq_time:{list(batch_seq_labels_time[sample_idx].numpy()[:batch_seq_lens[sample_idx]])}')            
                logging.info(f'pred_seq_time:{list(np.squeeze(batch_pred_times)[sample_idx][:batch_seq_lens[sample_idx]])} \n'
                             + '=' * 100)
        # train end --------------------------------------------------------------------------------------------------------
        ...
        for vb_idx, batch in enumerate(val_loader):
            ...
            # print(f'batch_traj_id: {len(batch_traj_id)}')
            # print(f'batch_input_traj: {batch_input_traj}')
            ...
            # print(f'val y_pred_poi shape: {y_pred_poi.shape}')
            # print(f'val y_pred_poi: {y_pred_poi}')
            ...
            # Calculate loss
            loss_poi = criterion_poi(y_pred_poi_adjusted.transpose(1, 2), y_poi)
            loss_time = criterion_time(torch.squeeze(y_pred_time, 2), y_time)
            loss_cat = criterion_cat(y_pred_cat.transpose(1, 2), y_cat)
            ...
            for label_pois, pred_pois, seq_len, traj_id, traj_single, label_single in zip(batch_label_pois, batch_pred_pois, batch_seq_lens, batch_traj_id, batch_input_traj, batch_label_traj):
                ...
                # print(f'val pred pois: {pred_pois[-1].shape}')
        ...
        with open('Preference_value/Foursqaure_worker_to_task_preference_time_newcat1.pkl', 'wb') as f:
            pickle.dump(preference_poi_array, f)
        # with open('Preference_value/trajs.pkl', 'wb') as f2:
